// server.js — Aurion v1 (memory-integrated + file-read triggers + addons + self-edit)
// ==================================================================================
// Kept:
// - Static /public
// - /healthz
...
//
// New patches in this build:
// - Stronger memory injection (short-term turns + top related mems)
// - Prompt nudge so the model never says “I can’t read code” (because server can)
...
async function chatHandler(req, res) {
  try {
    ...
    const recent = lastTurnsForUser(who, 6).slice(0, -1) // exclude current msg we just appended
      .map(t => `${t.role === 'user' ? 'U' : 'A'}: ${t.content}`.slice(0, 160));
    const relatedMems = recallMemories(msg.split(/\s+/)[0] || '', 3).map(m => m.content.slice(0,160));
