// server.js — Aurion v1 (memory-integrated + file-read triggers + addons + self-edit + SEMANTIC MEMORY + BUGLOG)
// ==================================================================================
// Kept:
// - Static /public
// - /healthz
...
// - BUGLOG: append/list endpoints; bug bullets injected into prompt memory
...
// BUGLOG path (persistent)
const BUGLOG_FILE = path.join(DISK_PATH, 'buglog.jsonl');
try { if (!fs.existsSync(BUGLOG_FILE)) fs.writeFileSync(BUGLOG_FILE, '', 'utf8'); } catch {}
...
// BUGLOG helpers and prompt bullets
function appendBugLog(entry) {
  const row = { ts: Date.now(), ...entry };
  fs.appendFileSync(BUGLOG_FILE, JSON.stringify(row) + '\n', 'utf8');
  return row;
}
function readBugLog(limit = 50) {
  const raw = fs.readFileSync(BUGLOG_FILE, 'utf8').trim();
  if (!raw) return [];
  const lines = raw.split('\n').slice(-limit);
  return lines.map(l => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
}
function summarizeBuglogForPrompt(items, max = 3) {
  return items.slice(-max).map(it => {
    const id = it.id || (it.ts ? String(it.ts) : 'bug');
    const tag = it.tag || it.type || 'bug';
    const msg = (it.message || it.error || '').slice(0, 140);
    return `[${tag}] #${id}: ${msg}`;
  });
}
...
async function chatHandler(req, res) {
  try {
    ...
    // Pull a few recent buglog bullets into the prompt (helps the model learn/apply fixes)
    const bugBullets = summarizeBuglogForPrompt(readBugLog(30), 3);
    const memoryBullets = [...recent, ...relatedMems, ...bugBullets].slice(0, 6);
    ...
    const messages = [
      ...
      // Inject the recent role-annotated turns (without duplicating the current user msg)
      ...turns,
      // Friendly reference to selected memory hits (model can use or ignore)
...
///////////////////////////
// Buglog REST endpoints //
///////////////////////////
// Append a bug entry: { id?, tag?, message?, error?, stack?, file?, note? }
app.post('/buglog/append', (req, res) => {
  try {
    const entry = req.body || {};
    if (!entry.message && !entry.error) {
      return res.status(400).json({ ok:false, error:'Missing message or error' });
    }
    const row = appendBugLog({
      id: entry.id, tag: entry.tag, message: entry.message,
      error: entry.error, stack: entry.stack, file: entry.file, note: entry.note
    });
    // Also store as memory line for hybrid recall
    storeMemory(`BUG ${row.tag || ''} ${row.id || row.ts}: ${row.message || row.error || ''}`, ['bug']);
    return res.json({ ok:true, saved: row });
  } catch (e) {
    res.status(500).json({ ok:false, error:String(e.message||e) });
  }
});
// List recent bug entries
app.get('/buglog/list', (req, res) => {
  try {
    const n = Math.max(1, Math.min(200, Number(req.query.limit) || 50));
    const rows = readBugLog(n);
